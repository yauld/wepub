...
浏览器打开终端输出的本地地址，默认是 <http://127.0.0.1:4173>。点击「打开本地文档」，选择 Markdown 或 Notebook 即可。文档引用的相对图片会从文档所在目录自动解析，无需逐张上传。

`npm run dev` 会启动同一个本地服务来承载前端页面和后端 API；如果默认端口被占用，会自动尝试后续端口。也可以手动指定：

```bash
npm run dev -- --port=4174
```

「打开本地文档」会使用 wepub 内置的本地文件浏览器，直接浏览目录并选择 `.md`、`.markdown` 或 `.ipynb` 文件；选中文档后，关联图片仍会从文档所在目录自动解析。
